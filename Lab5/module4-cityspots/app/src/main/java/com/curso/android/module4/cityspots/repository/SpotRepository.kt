...
import com.curso.android.module4.cityspots.utils.PhotoCaptureResult
...
class SpotRepository(
     ...
     */
    suspend fun capturePhoto(imageCapture: ImageCapture): PhotoCaptureResult {
    ...
    suspend fun createSpot(imageCapture: ImageCapture): CreateSpotResult {
        ...
        return when (val captureResult = capturePhoto(imageCapture)) {

            is PhotoCaptureResult.Error -> {
                CreateSpotResult.PhotoCaptureFailed(captureResult.error)
            }

            is PhotoCaptureResult.Success -> {

                val photoUri = captureResult.uri
                ...
                CreateSpotResult.Success(spot.copy(id = id))
            }
        }
    }
    ...
    suspend fun deleteSpot(spot: SpotEntity): DeleteSpotResult {
        return try {
            ...
            DeleteSpotResult.Success
        } catch (e: Exception) {
            DeleteSpotResult.Error("Error eliminando spot: ${e.message}")
        }
    }
    ...
    sealed class DeleteSpotResult {
        ...
        data class Error(val message: String) : DeleteSpotResult()
    }
