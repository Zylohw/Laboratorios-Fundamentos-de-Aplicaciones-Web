...
sealed class CaptureError {
    ...
    data class Unknown(val message: String?) : CaptureError()
}

sealed class PhotoCaptureResult {
    data class Success(val uri: Uri) : PhotoCaptureResult()
    data class Error(val error: CaptureError) : PhotoCaptureResult()
}
            ...
            imageCapture.takePicture(
                ...
                object : ImageCapture.OnImageSavedCallback {
                    ...
                    override fun onImageSaved(outputFileResults: ImageCapture.OutputFileResults) {
                        continuation.resume(
                            PhotoCaptureResult.Success(Uri.fromFile(photoFile))
                        )
                    }
                    ...
                    override fun onError(exception: ImageCaptureException) {
                        ...
                        val mappedError = when (exception.imageCaptureError) {
                            ...
                            ImageCapture.ERROR_FILE_IO ->
                                ...
                                CaptureError.Unknown(exception.message)
                        }

                        continuation.resume(
                            PhotoCaptureResult.Error(mappedError)
                        )
                    }
                }
            )
